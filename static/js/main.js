...
function init() {
    ...
    //
    var colorgrid = new THREE.Color("rgba(255,255,255,0.1)")
    grid = new THREE.GridHelper( 200, 20, 0xffffff, colorGrid = colorgrid );
    grid.name = "whole_grid"
    scene.add( grid );
    ...
    document.getElementById( 'getpreds' ).addEventListener( 'click', getPointRCNNLabels, false );
...
function getPointRCNNLabels(filename) {
    if (!enable_pointrcnn_predictions) {return;}
    $.ajax({
            url: '/getPointRCNNLabels',
            contentType: 'application/json;charset=UTF-8',
            success: function(response) {
                //We get the json file with the whole bbox multidim list
                make_predicted_bboxes(scene, response.data);
            },
            error: function(error) {
                console.log(error);
            }
...
function update_footer(pos) {
    ...
    if (isRecording) {
        ...
        reminder_text = "Unlock movement to continue annotating";
    }
...
function draw_cuboid_test(width, height, depth){
    var cuboid = new THREE.CubeGeometry(width, height, depth, 1, 1, 1);
    var material = new THREE.MeshLambertMaterial({ color: 0xFF0000 });
    var cube = new THREE.Mesh(cuboid, material);
    cube.position.y = 5
    scene.add(cube);
}
...
//     if (!isRecording) {
//         alert("Unlock movement to change modes");
//     }
// }
